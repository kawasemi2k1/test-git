...
HuffmanTreeArray makeHuffmanTreeArray(Coding htable[]); // huffmantree2array
...
{
	int i, byte, bit;
	byte = nbit / 8; // xau duoc ma hoa hien co bao nhieu byte
	bit = nbit % 8; // vi tri bit cuoi dung trong xau dang duoc ma hoa
	if (val == 0)
	{
		huffman[byte] &= ~(1 << bit);
	}
...
HuffmanTreeArray makeHuffmanTreeArray(Coding htable[]) // Huffman2Array
{
